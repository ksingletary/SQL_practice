...
SELECT MAX(reviews), app_name FROM analytics WHERE id <= 5 GROUP BY app_name;
...
SELECT app_name, price, rating FROM analytics WHERE price = (SELECT MAX(price) FROM analytics WHERE rating < 3) AND rating < 3 GROUP BY app_name, price, rating;

-- Find all apps with a min install not exceeding 50, that have a rating. Order your results by highest rated first.

SELECT app_name, min_installs, rating FROM analytics WHERE min_installs <= 50 GROUP BY app_name, min_installs, rating HAVING rating > 0;
...
SELECT app_name, reviews FROM analytics WHERE reviews = (SELECT MAX(reviews) FROM analytics) AND WHERE price BETWEEN .10 AND 1.00 GROUP BY app_name, reviews ORDER BY MAX(reviews);
...
SELECT app_name, price FROM analytics WHERE price = (SELECT MAX(price) FROM analytics) GROUP BY app_name, price;
